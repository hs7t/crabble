# Crabble

A fast-paced word game where you unscramble strings of words.

> [!NOTE]
> **Hi, reviewer!** I commit a lot - I apologise in advance. I've tried to take
> some steps to make reviewing easier: I'm committing less and have started
> working in different branches so I can group related commits into pull requests.
> And I've squashed commits in the same pull requests together (that's why you
> may see some extremely broken time).
> So, please, when reviewing, don't start with the commit history, 
> but with the [Pull Requests](https://github.com/hs7t/crabble/pulls?q=is%3Apr+is%3Aclosed)
> tab instead!

## Usage
### Playing with the keyboard
- Use `Q` and `W` to select previous and next word blocks, respectively.
- Hit `Space` after selecting a word block to pick it up. Use your left and
    right arrow keys (`←`, `→`) to move it to the right place, and hit `Space`
    again to set it down. 

## In use

## Credits
- - u/PizzaTucker's [neat trick to use an emoji as a favicon](https://www.reddit.com/r/webdev/comments/w8gx24/here_is_a_neat_trick_i_found_to_use_an_emoji_as_a/)